    ...
    public partial class frmFinanceDashBoard : Form
    {
        ...
        private void printToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //    }
                //    else
                //     if (gstno[0].PO_Format == "PO2")
                //    {
                //        rep = new MaterialManagement.Transactions.rptPurchaseOrder_New();
                //    }
                //}
                //else
                //{
        ...
        public void bindDashBoard()
        {
            ...
            var PR = (from data in db.ShowPymtRequestList_forApproval(logIn.company, logIn.fy_Start_Date, logIn.fy_End_Date, logIn.BU_ID) select data).ToList();
            ...
            chart1.Series["Purchase_Value"].XValueMember = "eMonth";
            chart1.Series["Purchase_Value"].YValueMembers = "Amount";
            chart1.DataBind();
...
&& s.Grn_Date <= logIn.fy_End_Date
        ...
        private void approveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            for (int i = 1; i < sfDataGrid1.RowCount; i++)
            {
                ...
                //{
                var rowData = sfDataGrid1.GetRecordAtRowIndex(i);
                var mappingName = sfDataGrid1.Columns["Req_No"].MappingName;
                    ...
                    //{
                        ...
                        if (uRole.Count > 0)
                        {
                            if (uRole[0].Approve_Role == true)
                            {
                                ...
                                var ci = db.Payment_Request_Masters.Where(w => w.Voucher_No == OrdNo && w.Company_ID == logIn.company).FirstOrDefault();
                                {
                            ...
                            {
                                MessageBox.Show("You Have No Permissions to Approve The Payment Requisitions");
                                return;
                            }
                        }
