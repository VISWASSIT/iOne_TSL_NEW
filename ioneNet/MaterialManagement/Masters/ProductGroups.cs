...
namespace ioneNet.Masters
{
    public partial class ProductGroups : Form
    {
        ...
        public void bindGroups()
        {
            try
            {
                var p = (from s in db.ProductGroup_Lists where s.status == "Active" && s.Company_ID == logIn.company
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Select Product Type", "Group Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    comboBox1.Focus();
                    return;
                }
                else
        ...
        public void Save()
        {
            try
            {
                    ...
                    if(txtGroupID.Text !="")
                {
                            ...
                            var c = db.Product_Groups.Where(w => w.ID == Convert.ToInt32(txtGroupID.Text)).FirstOrDefault();
                        {
                            //c.ID = Convert.ToInt32(txtCityId.Text.ToString());
                            c.Prod_Group_Name = (txtGroupName.Text == "") ? "" : (txtGroupName.Text);
                            ...
                            c.Status_ID = 1;
                            c.Prod_Type = Convert.ToInt32(comboBox1.SelectedValue.ToString()); ;
                ...
                else
                {
                    ...
                    //{
                        Product_Group ci = new Product_Group();
                    ci.Prod_Group_ID = (txtGroupID.Text == "") ? "" : (txtGroupID.Text);
                    ci.Prod_Group_Name = (txtGroupName.Text == "") ? "" : (txtGroupName.Text);
                    ...
                    ci.Prod_Type = Convert.ToInt32(comboBox1.SelectedValue.ToString()); ;
                    ...
                    ci.Status_ID = 1;
                    db.Product_Groups.InsertOnSubmit(ci);
                        db.SubmitChanges();
           ...
           try
           {
                if (txtGroupID.Text != "")
                {
                        ...
                        if (result == DialogResult.Yes)
                        {
                            ...
                            SqlCommand cmd1 = new SqlCommand("delete  from [Product_Groups] where id =@ProdID and Company_ID = @compID", con);
