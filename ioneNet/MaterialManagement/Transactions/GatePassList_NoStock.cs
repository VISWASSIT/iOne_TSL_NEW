...
{
    public partial class GatePassList_NoStock : Form
    {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (cellVaue1.ToString() != "")
                    {
                        ...
                        if (uRole.Count > 0)
                        {
                            ...
                            {
                                var ci = db.GatePass_Master_NonStockItems.Where(w => w.Id == Convert.ToInt32(custId) && w.Company_ID == logIn.company).FirstOrDefault();
                                {
                                    ci.isDeleted = true;
        ...
        public void BindOrderslist()
        {
            try
            {
                var d = (from data in db.SP_ShowGatepasslist_NonStockItems(logIn.company, logIn.fy_Start_Date, logIn.fy_End_Date,logIn.BU_ID) select data).ToList();
        ...
        private void approveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
              ...
              // boolen GPType = cellVaue1;
                ...
                rep.RecordSelectionFormula = "{GatePass_Master_NonStockItems.Id} = " + GPID + "  and { GatePass_Master_NonStockItems.Company_ID} = " + logIn.company + "";
        ...
        private void eMailToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                filepath = path;
                var Email = (from c in db.GatePass_Master_NonStockItems
                ...
                if (cellVaue.ToString() != "")
                {
                    ...
                    if (uRole.Count > 0)
                    {
                        if (uRole[0].View_Role == true)
                        {
                            ...
                            var = "1";
                            editMode = true;
                            MaterialManagement.Transactions.GatePasReturnable_NoStock frm = new MaterialManagement.Transactions.GatePasReturnable_NoStock();
