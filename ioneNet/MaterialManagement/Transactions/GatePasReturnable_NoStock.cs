...
{
    public partial class GatePasReturnable_NoStock : Form
    {
        ...
        public GatePasReturnable_NoStock()
        {
            InitializeComponent();
        }
        ...
        public void save()
        {
            try
            {
                ...
                if (txtVchNo.Text != "")
                {
                    ...
                    db.sp_Select_Delete_Gatepass_NonStockItems(logIn.company, txtVchNo.Text,logIn.BU_ID);
                }
                else
                {
                    AutoincrementId();
                ...
                GatePass_Master_NonStockItem S = new GatePass_Master_NonStockItem();
                {
                    ...
                    db.GatePass_Master_NonStockItems.InsertOnSubmit(S);
                    db.SubmitChanges();
                }
                ...
                {
                GatePass_NonStockitems_Child GP = new GatePass_NonStockitems_Child();
                var d1 = (from a in db.GatePass_Master_NonStockItems where a.GateVch_No == txtVchNo.Text && a.isDeleted ==false && a.Non_Returnable == checkBox1.Checked && a.Company_ID == logIn.company && a.BU_ID == logIn.BU_ID select new { a.Id }).ToList();
                ...
                db.GatePass_NonStockitems_Childs.InsertOnSubmit(GP);                    
                db.SubmitChanges();
                }
        ...
        public void AutoincrementId()
        {
            try
            {
                    ...
                    var auto = db.Sp_autoincrement_GatePass_NonStockItems(logIn.company, logIn.BU_ID, logIn.fy_Start_Date, logIn.fy_End_Date);
        ...
        private void GatePasReturnable_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (GatePassList_NoStock.var == "0")
                {
                    if (GatePassList_NoStock.editMode == true)
                    {
                        bindedit();
                    }
                }
                else
                if (GatePassList_NoStock.var == "1")
                {
                    if (GatePassList_NoStock.editMode == true)
                    {
                        bindedit();
        ...
        public void bindedit()
        {
            try
            {
                ...
                txtVchNo.Text = GatePassList_NoStock.SO_No;
                var d = (from po in db.GatePass_Master_NonStockItems                         
                            where po.GateVch_No == GatePassList_NoStock.SO_No && po.Company_ID == logIn.company && po.BU_ID ==logIn.BU_ID
                            select po).ToList();
                ...
                var p = (from po in db.GatePass_NonStockitems_Childs                     
                         join g1 in db.GatePass_Master_NonStockItems on po.GP_Master_ID equals g1.Id      
                            where po.GP_Master_ID == GatePassList_NoStock.gp_id && po.Company_ID == logIn.company && g1.BU_ID == logIn.BU_ID
                            select new
                            {
                                //po.Product_code,
                                po.S_No,
            ...
            try
            {
                ...
                if (columnName == "Product_Name" && R1.Cells["Product_Name"].Value != null)
                {
                    ...
                    if (getProductName == null)
                    {
                        ...
                        {
                            R1.Cells["Prod_Code"].Value = dgProducts.CurrentCell.RowIndex + 1;
                            R1.Cells["Int_Prod_Code"].Value = "NA";
                            if (R1.Cells["S_no"].Value == null || R1.Cells["S_no"].Value.ToString() == "")
                            {
                                R1.Cells["S_no"].Value = dgProducts.Rows.Count - 1;
