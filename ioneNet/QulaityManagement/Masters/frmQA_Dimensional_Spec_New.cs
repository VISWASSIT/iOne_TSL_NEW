...
{
    public partial class frmQA_Dimensional_Spec_New : Form
    {
        DataClasses1DataContext db = new DataClasses1DataContext();
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iOneConnection"].ConnectionString);

        public frmQA_Dimensional_Spec_New()
        {
            InitializeComponent();
        }
        ...
        private void frmQA_Chemical_Spec_Load(object sender, EventArgs e)
        {
            ...
            //Status
            var pStatus = (from m in db.Products
                           join p in db.Attributes_Prod_Types on m.Prod_Type_Id equals p.Prod_Type_Id
                           where p.Prod_Type == "Finished Goods" && m.Company_ID == logIn.company
                           select new { m.prod_ID, m.Prod_Name }).Distinct().ToList();
            if (pStatus.Count > 0)
            {
                cmbMtrlGrade.DataSource = pStatus;
                cmbMtrlGrade.ValueMember = "prod_ID";
                cmbMtrlGrade.DisplayMember = "Prod_Name";
            }
        }
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (txtRevNo.Text == null || txtRevNo.Text =="")
            {
            ...
            if ((from a in db.QA_Dimensional_Specs where a.Company_id == logIn.company && a.Prod_ID == Convert.ToInt32(cmbMtrlGrade.SelectedValue) && a.Rev_No == Convert.ToInt32(txtRevNo.Text) select a).Count() > 0)
            {
                db.sp_Delete_Mtrl_Dimensionla_Specs (Convert.ToInt32(cmbMtrlGrade.SelectedValue), logIn.company, "Dimensional", Convert.ToInt32(txtRevNo.Text));
            }

            for (int i = 0; i < dgSpecs.RowCount - 1; i++)
            {
                QA_Dimensional_Spec SC = new QA_Dimensional_Spec();
                ...
                SC.Prod_ID = Convert.ToInt32(cmbMtrlGrade.SelectedValue);
                ...
                db.QA_Dimensional_Specs.InsertOnSubmit(SC);
                db.SubmitChanges();               
            }
        ...
        public void addItems(AutoCompleteStringCollection coll)
        {
            try
            {
                ...
                {
                    var Prodname = (from d in db.QA_Test_Parameters where d.Test_Group == "Dimensional" select new { d.Test_Parameter }).ToList();
        ...
        private void dgSpecs_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (columnName == "Parameter_Name")
                {
                    ...
                    var getProductName = (from s in db.QA_Test_Parameters where s.Test_Parameter == R1.Cells["Parameter_Name"].Value.ToString()
        ...
        private void cmbMtrlGrade_Leave(object sender, EventArgs e)
        {
            try
            {
                if (cmbMtrlGrade.Text != "")
                {
                    ...
                    int grn_id = 0;
                    var da = (from obj in db.sp_Get_QA_Dimensional_Spec_LatestRevData(Convert.ToInt32(cmbMtrlGrade.SelectedValue), "Dimensional")
                    ...
                    if (da.Count > 0)
                    {
                        ...
                        var ds = (from obj in db.QA_Dimensional_Specs
                                  join a in db.QA_Test_Parameters on obj.Parameter_ID equals a.id
                                  where obj.Prod_ID == Convert.ToInt32(cmbMtrlGrade.SelectedValue)
                                  && obj.Rev_No == Convert.ToInt32(txtRevNo.Text) && a.Test_Group == "Dimensional"
                                  select new
                                  {
                                      parameter_id = obj.Parameter_ID,
                                      Parameter_Name = a.Test_Parameter,
        ...
        private void cmbMtrlGrade_LocationChanged(object sender, EventArgs e)
        {
