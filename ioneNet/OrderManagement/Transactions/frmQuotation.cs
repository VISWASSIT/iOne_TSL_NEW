    ...
    public partial class frmQuotation : Form
    {
        ...
        private void frmNewOrder_Load(object sender, EventArgs e)
        {
            ...
            if (ListOfQuotes.editMode == true)
            {
                ...
                //Bind Previos Version Quotes
                //var dm1 = (from s in db.Sale_Quotation_Masters
                //           where s.Quot_NO == txtSoNo.Text && s.Company_ID == logIn.company && s.bu_id == logIn.BU_ID && s.is_amended == true
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("Quotation Number Should Not Be Empty", "Quotation", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    txtSoNo.Focus();
                    return;
                }
                if (CmbBuyerName.Text == string.Empty)
                {
                    MessageBox.Show("Customer Name Should Not Be Empty", "Quotation", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    CmbBuyerName.Focus();
                    return;
                }               
                else if (cmbTaxClass.Text == string.Empty)
                {
                    MessageBox.Show("Tax Class Should Not Be Empty", "Quotation", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    cmbTaxClass.Focus();
                    return;
                }                
                else if (txtEnqNo.Text == string.Empty)
                {
                    MessageBox.Show("Select Enquiry No,");
                    txtEnqNo.Focus();
                    return;
                }
                ...
                {
                    MessageBox.Show("Atleast One Record To Be Entered To Proceed", "Quotation", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
        ...
        public void bindDroupDown_Lookup()
        {
            try
            {
                ...
                //Packing
                //var Pack = (from m in db.Attributes_Datas where m.Head_Name == "Packing" select new { m.ID, m.Descr }).Distinct().ToList();
                //if (Pack.Count > 0)
                //{
                //    cmbPacking.DataSource = Pack;
                ...
                //Bind Tax Class
                var bindLoc = (from m in db.Costing_Units
                ...
                if (bindLoc.Count > 0)
                {
                    cmbTaxClass.DisplayMember = "BU_ShortName";
        ...
        public void AutoincrementId()
        {
            try
            {
                ...
                var result = db.Sp_autoincrement_SaleQuotation_TSL (logIn.company,logIn.BU_ID, logIn.fy_Start_Date, logIn.fy_End_Date);
                txtSoNo.Text = result.FirstOrDefault().Quot_no;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
                ...
                if (CmbBuyerName.Text != "")                   
                {
                    ...
                    else
                    {
                        ...
                        if (State.Count > 0)
                        {
                            ...
                            cmbSaleOffice.SelectedValue = State[0].Area_Region;
        ...
        private void txtEnqNo_Enter(object sender, EventArgs e)
        {
            try
            {
                ...
                AutoCompleteStringCollection DataColl = new AutoCompleteStringCollection();
                AddEnqNos(DataColl);
                txtEnqNo.AutoCompleteCustomSource = DataColl;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void txtEnqNo_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtEnqNo.Text != "NA" && txtEnqNo.Text != "")
                {
                    String myString = "";
                    int QuoteMasterID = 0;
                    ...
                    if (da.Count > 0)
                    {
                        ...
                        dpQuotDate.Text = da[0].Enq_Date.ToString();
                        //bindCustomer();
                        ...
                        txtCustPoNo.Text = da[0].Cust_RefNo;
                        ...
                        if (da[0].Payment_Terms == null)
                        {
                            cmbPaymentTerms.SelectedValue = 36;
                        }
                        else
                        {
                            cmbPaymentTerms.SelectedValue = da[0].Payment_Terms;
                        }
                        ...
                        {
                            cmbSaleExecutive.SelectedValue = 36;
                        }
                        else
                        {
                        ...
                        if (da[0].Delivery_Location != null)
                        {
                            txtCity.Text = da[0].Delivery_Location;
                        }
                        txtSplInstructions.Text = da[0].Special_Instructions;
                        

                    }
                    //Get Products
                    SqlCommand cmd2 = new SqlCommand("SP_GetEnqDetails_ForQuote", con);
                    cmd2.CommandType = CommandType.StoredProcedure;
                    cmd2.Parameters.AddWithValue("@compname", logIn.company);

                    cmd2.Parameters.AddWithValue("@EnqNo", txtEnqNo.Text);
                    SqlDataAdapter da2 = new SqlDataAdapter(cmd2);
        ...
        {
            //Payment Terms
            string pt = cmbPaymentTerms.Text;
            var PTerms = (from m in db.Attributes_Datas where m.Head_Name == "Payment Terms" select new { m.ID, m.Descr }).Distinct().ToList();
            if (PTerms.Count > 0)
            {
                cmbPaymentTerms.DataSource = PTerms;
        ...
        public void Save()
        {
            try
            {
                ...
                myString = txtSoNo.Text;
                if ((from u in db.Sale_Quotation_Masters where u.Quot_NO == myString && u.Company_ID == logIn.company && u.bu_id == logIn.BU_ID select u).Count() > 0)
                {
                    if (ListOfQuotes.var == "1")
                    {
                        var result = db.Sp_autoincrement_SaleQuotation_AmendNo(logIn.company, logIn.BU_ID, ListOfQuotes.SO_No);
                        txtAmendNo.Text = result.FirstOrDefault().Quot_Amend_no;

                        //Set Previous Version Quote Status as Amended
                        var ci = db.Sale_Quotation_Masters.Where(w => w.Quot_NO == myString && w.Quot_Amend_No == AmendNo && w.Company_ID == logIn.company && w.bu_id == logIn.BU_ID).FirstOrDefault();
                        {
                    ...
                    else
                    {
                        ...
                        db.sp_Quot_Delete(ReceordMasterID.ToString(), logIn.company);
                    }
                }
                else
                {
                ...
                Sale_Quotation_Master S = new Sale_Quotation_Master();
                {
                    S.Quot_NO = myString;
                    S.Quot_Date = dpSODate.Value;
                    S.EnqNo = txtEnqNo.Text;
                    ...
                    S.CustomerRefDoc = (txtCustPoNo.Text == "") ? "" : txtCustPoNo.Text;                 
                    S.Delivery_Location = (txtCity.Text == "") ? "" : txtCity.Text;

                    S.Quot_Amend_No = (txtAmendNo.Text == "") ? "" : txtAmendNo.Text;
                    S.Quot_Amend_Date = dtAmendDate.Value;
                    S.UT_Required = Convert.ToInt32(txtUT.Text);
                    
                    S.RM_Price_Considered = (txtMOQ.Text == "") ? Convert.ToDecimal("0") : Convert.ToDecimal(txtMOQ.Text);

                    S.Export_Quote = (chkExport.Checked == true) ? true : false;
                    ...
                    //if (cmbSaleExecutive.SelectedValue == null)
                    //{
                    //    S.SaleExecutive = 0;
                    //}
                    //else
                   ...
                   // S.Repeat_Order = (chkRepeatOrder.Checked == true) ? true : false;
                    ...
                    S.Spl_Instructions = (txtSplInstructions.Text == "") ? "" : txtSplInstructions.Text;
                     S.Quote_Subject = (txtQuoteSubject.Text == "") ? "" : txtQuoteSubject.Text;
                     S.Quote_Validity = txtValidity.Text;
                    ...
                    db.Sale_Quotation_Masters.InsertOnSubmit(S);
                    db.SubmitChanges();
                }
                        //db.Transaction = transaction;
                for (int i = 0; i < dgProducts.RowCount - 1; i++)
                {
                    Sale_Quotation_Child SC = new Sale_Quotation_Child();
                    var d1 = (from a in db.Sale_Quotation_Masters where a.Quot_NO == myString && a.Quot_Amend_No == txtAmendNo.Text && a.Company_ID == logIn.company && a.bu_id == logIn.BU_ID select new { a.Id }).ToList();
                    SC.Quot_Master_ID = d1[0].Id;
                    SC.Quot_NO = myString;
                    ...
                    if (dgProducts.Rows[i].Cells["Quot_Line_Item_No"].Value == DBNull.Value || dgProducts.Rows[i].Cells["Quot_Line_Item_No"].Value == null || dgProducts.Rows[i].Cells["Quot_Line_Item_No"].Value == "")
                    {
                        SC.Quote_Item_No = i + 1;
                    }
                    else
                    {
                        SC.Quote_Item_No = Convert.ToInt32(dgProducts.Rows[i].Cells["Quot_Line_Item_No"].Value);
                    }

                    SC.Company_ID = logIn.company;
                    db.Sale_Quotation_Childs.InsertOnSubmit(SC);
                }
                db.SubmitChanges();
                //transaction.Commit();               
        ...
        public void bindedit()
        {
            try
            {
                ...
                var da = (from obj in db.getQuote_Master_data(logIn.company, logIn.BU_ID, txtSoNo.Text)
                          select new
                          {
                              obj.Id,
                              obj.Quot_NO,
                ...
                if (da.Count > 0)
                {
                    ...
                    txtSoNo.Text = da[0].Quot_NO.ToString();
                    dpSODate.Text = da[0].Quot_Date.ToString();
                    //bindCustomer();
                    CmbBuyerName.SelectedValue = da[0].BuyerName;
                    //cmbTaxClass.SelectedValue = da[0].Tax_Class;
                  //  CmbConsigneeName.SelectedValue = da[0].ConsigneeName;
                  //  txtCustGSTNo.Text = da[0].Cust_GST_No;
                    txtAmendNo.Text = da[0].Quot_Amend_No;
                    dtAmendDate.Text = da[0].Quot_Amend_Date.ToString();
                    ...
                    dpQuotDate.Text = da[0].EnqDate.ToString();
                  ...
                  //  dpPODate.Text = da[0].PODate.ToString();
                   ...
                   // txtFreight_Amt.Text = da[0].Frieght_Amount.ToString();
                    ...
                    txtValidity.Text = da[0].Quote_Validity;
                    txtCustomeContact.Text = da[0].Customer_Contact;
                    txtQuoteSubject.Text = da[0].Quote_Subject;
                    txtMOQ.Text = da[0].RM_Price_Considered.ToString();
                ...
                var dm1 = (from s in db.Sale_Quotation_Childs
                           join p in db.Products on s.Prod_Code equals p.prod_ID into prod
                           from prodcode in prod.DefaultIfEmpty()
                           join u in db.UoM_Masters on prodcode.Prod_Primary_UOM_Id equals u.UOM_ID
                           where s.Quot_Master_ID == ReceordMasterID && s.Company_ID == logIn.company
                           select new
                           {
                               Item_Code = s.Prod_Code,
                               Quot_Line_Item_No = s.Quote_Item_No,
                               Prod_Code = prodcode.Prod_Code,
                               Item_Description = prodcode.Prod_Name,
