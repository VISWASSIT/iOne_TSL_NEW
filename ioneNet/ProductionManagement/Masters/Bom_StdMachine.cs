...
namespace ioneNet.ProductionManagement.Masters
{
    ...
    public partial class Bom_StdMachine : Form
    {
        DataClasses1DataContext db = new DataClasses1DataContext();
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iOneConnection"].ConnectionString);

        public Bom_StdMachine()
        {
            InitializeComponent();
        }
        ...
        public void BindMasters()
        {
            try
            {
                ...
                //Bind Products
                var sa = (from a in db.Project_code_Masters
                          where a.Company_ID == logIn.company
                          select new { a.Project_Code,a.id }).ToList();
                if (sa.Count > 0)
                {
                    cmbProjectCode.DataSource = sa;
        ...
        private void dgbom_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Select Project / Machine Code");
                ...
                if (columnName == "Item_Name")
                {
                    if (R1.Cells["Item_Name"].Value != null)
                    {
                        ...
                        var getProductName = (from s in db.Products
                                              ...
                                              where s.Prod_Name == R1.Cells["Item_Name"].Value.ToString()
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                db.Sp_delete_BomProject(logIn.company,"STD", Convert.ToInt32(cmbProjectCode.SelectedValue),txtModule.Text);
        ...
        private void Save()
        {
            try
            {
                if ((from a in db.BOM_Projects where a.Company_ID == logIn.company && a.MO_No == "STD" && a.Project_Id == Convert.ToInt32(cmbProjectCode.SelectedValue) && a.Material_Group == Convert.ToInt32(cmbProdGroup.SelectedValue) && a.Module_No == txtModule.Text select a).Count() > 0)
                {
                    db.Sp_delete_BomProject(logIn.company, "STD", Convert.ToInt32(cmbProjectCode.SelectedValue), txtModule.Text);
                    for (int i = 0; i < dgbom.Rows.Count - 1; i++)
                    {
                        ...
                        bo1.Project_Code = cmbProjectCode.Text;
        ...
        private void GetMethod()
        {
            ...
            var sa = (from a in db.BOM_Projects
                      where a.Company_ID == logIn.company && a.Project_Code == cmbProjectCode.Text && a.MO_No == "STD" && a.Material_Group == Convert.ToInt32(cmbProdGroup.SelectedValue) && a.Module_No == txtModule.Text
                      select new
                      {
                          a.Project_Id,
                          a.Project_Code,
            ...
            if (sa.Count > 0)
            {
                ...
                //cmbProjectCode.Text = sa[0].Project_Code;
            ...
            var ca = (from sq in db.BOM_Projects
                      join b in db.Product_Groups on sq.RM_Group_Name equals b.ID
                      join u in db.UoM_Masters on sq.RM_UOM_ID equals u.UOM_ID
                      join p in db.Products on sq.RM_Item_ID equals p.prod_ID
                      where sq.Company_ID == logIn.company && sq.Project_Code == cmbProjectCode.Text && sq.MO_No == "STD" && sq.Material_Group == Convert.ToInt32(cmbProdGroup.SelectedValue) && sq.Module_No == txtModule.Text
                      orderby sq.Item_Sno
                      select new
                      {
                          SNo = sq.Item_Sno,
                          Prod_Code = sq.RM_Prod_Code,
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                ProductionManagement.Masters.Bom_Project_Search.frmName = "BOM-STD";
                ...
                if (obj.ShowDialog() == DialogResult.OK)
                {
                   // cmbMONo.Text = ProductionManagement.Masters.Bom_Project_Search.MoNO;
                    cmbProjectCode.SelectedValue = Convert.ToInt32(ProductionManagement.Masters.Bom_Project_Search.Proj_Code);
        ...
        private void cmdPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dgbom.Rows.Count > 0)
                {
                    ...
                    worksheet.Cells[3, 1] = "Production Name : " + cmbProjectCode.Text;
        ...
        public void AddProject(AutoCompleteStringCollection coll)
        {
            try
            {
                ...
                var Prodname = (from d in db.Project_Module_Masters
                                where d.Project_ID == Convert.ToInt32(cmbProjectCode.SelectedValue)
        ...
        private void btnImportBOM_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var bindGroups = (from m in db.BOM_Projects
                                      join g in db.Product_Groups on m.RM_Group_Name equals g.ID
                                      join u in db.UoM_Masters on m.RM_UOM_ID equals u.UOM_ID
                                      where m.Company_ID == logIn.company && m.Project_Id == Convert.ToInt32(cmbOldProjCode.SelectedValue)
                                      orderby m.Item_Sno
                                      select new
                                      {
                                          SNo = m.Item_Sno,                                          
                                          Item_Code =m.RM_Item_ID,
                                          Prod_Code = m.RM_Prod_Code,
                                          Item_Name =m.RM_Item_Name,
                                          Group_Name  = g.Prod_Group_Name,
                                          UOM = u.Uom_Descr,
                                          QtyReq = m.QtyReq

                                      }).Distinct();
                    SqlCommand cmd2 = (SqlCommand)db.GetCommand(bindGroups);
                    SqlDataAdapter da2 = new SqlDataAdapter(cmd2);
                    DataTable dtr = new DataTable();
                    da2.Fill(dtr);
                    if (dtr.Rows.Count >= 0)
                        dgbom.DataSource = dtr;
                    else
                    {
        ...
        private void txtModule_Leave(object sender, EventArgs e)
        {
            try
            {
                if (txtModule.Text != "")
                {
                    if ((from u in db.Project_Module_Masters where u.Project_ID == Convert.ToInt32(cmbProjectCode.SelectedValue) &&  u.Module_Name == txtModule.Text && u.Company_ID == logIn.company select u).Count() > 0)
                    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
             ...
             for (int i = 0; i < count; i++)
            {
                ...
                var d1 = (from a in db.Products where a.Prod_Code == DtSet.Rows[i]["Product Code"].ToString() select new { a.prod_ID, a.Prod_Primary_UOM_Id }).ToList();
                if (d1.Count > 0)
                {
                    dr["Item_Code"] = d1[0].prod_ID;
                ...
                dr["Item_Name"] = DtSet.Rows[i]["Product Name"].ToString();
                dr["Group_Name"] = DtSet.Rows[i]["Category"].ToString(); ;
                dr["UOM"] = DtSet.Rows[i]["UOM"].ToString();
                dr["QtyReq"] = DtSet.Rows[i]["Qty Required"].ToString();                 
                dt.Rows.Add(dr);
            }
            dgbom.DataSource = dt;
