...
{
    public partial class frmForge_Production_Forging_RFPL : Form
    {
        ...
        public frmForge_Production_Forging_RFPL()
        {
            InitializeComponent();
        }
        ...
        private void frmForge_Production_Forging_Load(object sender, EventArgs e)
        {
            ...
            if (frmForging_Production_ForgingList.editMode == true)
            {
                txtvchno.Text = frmForging_Production_ForgingList.SO_No;
                bindedit();
            }
            ...
            try
            {
               ...
               //int QuoteMasterID = 0;
                ...
                if (sa.Count > 0)
                {
                    ...
                    txtStartGasReading.Text = sa[0].GasReading_Start.ToString();
                    txtEndGasReading.Text = sa[0].GastReading_End.ToString();
                ...
                var ca = (from sq in db.Forging_Forging_Childs
                          ...
                          select new
                          {
                              ...
                              Rejection_Reason = sq.Rejection_Reason,
                              sq.Remarks
                          });
        ...
        private void dgJobCardData_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            try
            {
                ...
                if (tb3 != null && columnName == "Disposal")
                {
                    ...
                    if (tb3 != null && columnName == "Reasons for Delay")
                {
                    tb3.AutoCompleteMode = AutoCompleteMode.Suggest;
                    tb3.AutoCompleteSource = AutoCompleteSource.CustomSource;
                    AutoCompleteStringCollection DataColl = new AutoCompleteStringCollection();
        ...
        public void addItems(AutoCompleteStringCollection coll)
        {
            try
            {
                ...
                if (columnName == "Reasons for Delay")
                {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < dgJobCardData.Rows.Count - 1; i++)
                {
                    PlanQty = (dgJobCardData.Rows[i].Cells["Pending_Qty"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["Job_Card_Qty"].Value);
                    ProdQty = (dgJobCardData.Rows[i].Cells["Qty_Produced"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["Qty_Accepted"].Value);
                    if (dgJobCardData.Rows[i].Cells["Rejection_Reason"].Value == null)
                    {
...

                {
                    ...
                    if ((from a in db.Forging_ForgingReport_Masters where a.Company_ID == logIn.company && a.Report_No == txtvchno.Text select a).Count() > 0)
                    {
                        ...
                        pb.Plan_Ref_No = txtPlanningRef.Text;
                        ...
                        pb.GasReading_Start = (txtStartGasReading.Text == "") ? Convert.ToDecimal("00") : Convert.ToDecimal(txtStartGasReading.Text);
                        pb.GastReading_End = (txtEndGasReading.Text == "") ? Convert.ToDecimal("00") : Convert.ToDecimal(txtEndGasReading.Text);
                        ...
                        for (int i = 0; i < dgJobCardData.Rows.Count - 1; i++)
                        {
                            ...
                            SC.Prod_Rate = (dgJobCardData.Rows[i].Cells["No_Heats"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["No_Heats"].Value);
                            ...
                            SC.RF_No = (dgJobCardData.Rows[i].Cells["RF_No"].Value == null) ? "" : (dgJobCardData.Rows[i].Cells["RF_No"].Value).ToString();
                            SC.Input_Wt = (dgJobCardData.Rows[i].Cells["Input_Wt"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["Input_Wt"].Value);
                            SC.Forging_Wt = (dgJobCardData.Rows[i].Cells["Forging_Wt"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["Forging_Wt"].Value);
                            SC.Rejection_Reason = (dgJobCardData.Rows[i].Cells["Rejection_Reason"].Value == null) ? "" : (dgJobCardData.Rows[i].Cells["Rejection_Reason"].Value).ToString();
                            SC.Customer_Name = (dgJobCardData.Rows[i].Cells["Customer_Name"].Value == null) ? "" : (dgJobCardData.Rows[i].Cells["Customer_Name"].Value).ToString();
