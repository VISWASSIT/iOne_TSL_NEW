...
{
    public partial class ProductionReport_SubContractor : Form
    {
        DataClasses1DataContext db = new DataClasses1DataContext();
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iOneConnection"].ConnectionString);
        public static string DocNo, ItemCode, RecQty, Suppname;
        public ProductionReport_SubContractor()
        {
            InitializeComponent();
        }
        ...
        private void cmbReceipe_Leave(object sender, EventArgs e)
        {
            if (logIn.company == 16)
            {
                ...
                for (int i = 0; i < dgrmconsumed.Rows.Count; i++)
                {
                    ...
                    var stock = (from data in db.ShowItemWiseStockReport_SubContractor (logIn.company, Convert.ToInt32(dgrmconsumed.Rows[i].Cells["Item_ID"].Value), t,Convert.ToInt32(cmbCustomer.SelectedValue),logIn.BU_ID) select data).ToList();
                    if (stock.Count > 0)
                    {
            ...
            {
                ////DateTime t = dpdate.Value;
                ////var stock = (from data in db.GetBomItems(logIn.company, Convert.ToInt32(cmbpname.SelectedValue), Convert.ToInt32(cmbReceipe.Text), t)
        ...
        private void txtQtyAccepted_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DateTime t = dpdate.Value;
                    SqlCommand cmd = new SqlCommand("GetBomItems_Subcontractor", con);
                    ...
                    cmd.Parameters.AddWithValue("@Receipe", Convert.ToInt32(cmbReceipe.Text));
                    cmd.Parameters.AddWithValue("@fromDate", t);
                    cmd.Parameters.AddWithValue("@BatchQty", Convert.ToDecimal(txtQtyAccepted.Text));
                    cmd.Parameters.AddWithValue("@SubContractor", Convert.ToInt32(cmbCustomer.SelectedValue));
                    SqlDataAdapter da = new SqlDataAdapter(cmd);
        ...
        private void cmbpname_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if(cmbpname.Text!="")
                { 
                var sa = (from a in db.Bom_Data_Views where a.Bom_Item_ID == Convert.ToInt32(cmbpname.SelectedValue) && a.Company_ID == logIn.company select new { a.Prod_Group_Name, a.Uom_Descr, a.Bom_Batchsize, a.Bom_Item_UnitWt, a.TotComp }).ToList();
                if (sa.Count > 0)
                {
                ...
                var d = (from a in db.Products
                         ...
                         select new { b.Prod_Name, b.Prod_Scrap_Product_Id }).ToList();

                if (d.Count > 0)
                {
                    cmbScrapProduct.DataSource = d;
                    cmbScrapProduct.ValueMember = "Prod_Scrap_Product_Id";
