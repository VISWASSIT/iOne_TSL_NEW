...
{
    public partial class frmForge_Production_Grinding : Form
    {
        DataClasses1DataContext db = new DataClasses1DataContext();
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iOneConnection"].ConnectionString);

        public frmForge_Production_Grinding()
        {
            InitializeComponent();
        }
        ...
        private void frmForge_Production_Forging_Load(object sender, EventArgs e)
        {
            ...
            var pStatus = (from m in db.Forging_ProcessMasters where m.Company_ID == logIn.company && (m.Main_Process_ID == 4 || m.Main_Process_ID == 36) select new { m.ID, m.Process_Name }).Distinct().ToList();
            if (pStatus.Count > 0)
            {
        ...
        public void AutoincrementId()
        {
            try
            {
                ...
                var result = db.Sp_autoincrement_Forging_GrindingReport(logIn.company);
        ...
        private void dgJobCardData_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (columnName == "Rough_Grinding" || columnName == "Finishing_Qty")
                {
                    ...
                    if (R1.Cells["Job_Card"].Value != null)
                    {
                        ...
                        {
                            decimal Ideal_Hrs = (Convert.ToDecimal(R1.Cells["Finishing_Qty"].Value)) / Convert.ToDecimal(R1.Cells["PR"].Value);
                            //PR
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if ((from a in db.Forging_GrindingReports where a.Company_ID == logIn.company && a.Report_No == txtvchno.Text select a).Count() > 0)
                {
                    myString = txtvchno.Text;
                    db.Sp_delete_Production_Grinding(logIn.company, txtvchno.Text);    
                    //Save Child Data (Job Card)
                    for (int i = 0; i < dgJobCardData.Rows.Count - 1; i++)
                    {
                        Forging_GrindingReport SC = new Forging_GrindingReport();
                        ...
                        //SC.Rough_Grinding_Qty = (dgJobCardData.Rows[i].Cells["Rough_Grinding"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["Rough_Grinding"].Value);
                        SC.Finishing_qty = (dgJobCardData.Rows[i].Cells["Finishing_Qty"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["Finishing_Qty"].Value);                                         
                        SC.PR = (dgJobCardData.Rows[i].Cells["PR"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["PR"].Value);
                        SC.IPH = (dgJobCardData.Rows[i].Cells["Ideal_Hrs"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["Ideal_Hrs"].Value);
                        SC.Operator = (dgJobCardData.Rows[i].Cells["Operator_Name"].Value == null) ? "" : (dgJobCardData.Rows[i].Cells["Operator_Name"].Value).ToString();
                                              
                        
                        db.Forging_GrindingReports.InsertOnSubmit(SC);
                    }
                    db.SubmitChanges();
        ...
        private void btnFind_Click(object sender, EventArgs e)
        {
            try
            {
                ProductionManagement.frmForging_Production_OtherReportList obj = new frmForging_Production_OtherReportList();
                if (obj.ShowDialog() == DialogResult.OK)
                {
                    int Report_ID = 0;
                    txtvchno.Text = ProductionManagement.frmForging_Production_OtherReportList.Voucherno;
                    var sa = (from sq in db.Forging_GrindingReports
                    ...
                    if (sa.Count > 0)
                    {
                        ...
                        txtRemarks.Text = sa[0].Report_Remarks;
                    ...
                    var ca = (from sq in db.Forging_GrindingReports
                              ...
                              select new
                              {
                                  ...
                                  Finishing_Qty =sq.Finishing_qty,                                 
                                  sq.PR,
                                  Ideal_Hrs = sq.IPH,                                  
                                  Operator_Name = sq.Operator,  
