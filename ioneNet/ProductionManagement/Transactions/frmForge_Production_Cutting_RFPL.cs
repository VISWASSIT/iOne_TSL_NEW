...
{
    public partial class frmForge_Production_Cutting_RFPL : Form
    {
        DataClasses1DataContext db = new DataClasses1DataContext();
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iOneConnection"].ConnectionString);

        public frmForge_Production_Cutting_RFPL()
        {
            InitializeComponent();
        }
        ...
        private void dgJobCardData_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            try
            {
                ...
                if (tb3 != null && columnName == "Item Code")
                {
                    tb3.AutoCompleteMode = AutoCompleteMode.Suggest;
                    tb3.AutoCompleteSource = AutoCompleteSource.CustomSource;
                    AutoCompleteStringCollection DataColl = new AutoCompleteStringCollection();
                ...
                if (tb3 != null && columnName == "RF No")
                {
                    ...
                    if (tb3 != null && columnName == "Remarks")
                {
                    tb3.AutoCompleteMode = AutoCompleteMode.Suggest;
                    tb3.AutoCompleteSource = AutoCompleteSource.CustomSource;
                    AutoCompleteStringCollection DataColl = new AutoCompleteStringCollection();
        ...
        public void addItems(AutoCompleteStringCollection coll)
        {
            try
            {
                ...
                if (columnName == "MO NO")
                {
                    var Prodname = (from d in db.Forge_Cutting_Plans where d.Company_ID == logIn.company                                
                ...
                {
                    if (columnName == "Item Code")
                    {
                        var Prodname = (from d in db.Forge_Cutting_Plans where d.Mo_No == R1.Cells["Job_Card"].Value.ToString() select new { d.Prod_Code }).ToList();
                        DataTable dt = new DataTable();
                        dt.Columns.Add("Item_Code");
                        foreach (var item in Prodname)
                        {
                            dt.Rows.Add(item.Prod_Code);
                        }
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            coll.Add(dt.Rows[i][0].ToString());
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (columnName == "RF No")
                                {
                                    var Prodname = (from d in db.RF_Cutting_RM_Allotments where d.MO_No == R1.Cells["Job_Card"].Value.ToString() && d.Item_No == Convert.ToInt32(R1.Cells["Item_Code"].Value.ToString()) select new { d.RF_No }).ToList();
                                    DataTable dt = new DataTable();
                                    dt.Columns.Add("RF_No");
                                    foreach (var item in Prodname)
                                    {
                                        dt.Rows.Add(item.RF_No);
                                    }
                                    for (int i = 0; i < dt.Rows.Count; i++)
                                    {
                                        coll.Add(dt.Rows[i][0].ToString());
                                ...
                                else
                                {
                                    ...
                                    {
                                        var Prodname = (from d in db.Attributes_Datas where d.Head_Name == "Reasons for Delay" select new { d.Descr }).ToList().Distinct();
                                        DataTable dt = new DataTable();
        ...
        private void dgJobCardData_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (columnName == "Job_Card" && R1.Cells["Job_Card"].Value != null)
                {
                    ...
                    var getDetails = (from s in db.Forge_MO_Production_Status(logIn.company, R1.Cells["Job_Card"].Value.ToString(), 0,1)
                                      select new { s.Customer_Name}).FirstOrDefault();

                    if (getDetails != null)
                    {
                      //  y = Convert.ToDecimal(getDetails.QtyPending.ToString());
                        R1.Cells["Machine_Name"].Value = getDetails.Customer_Name.ToString();   
                        //
                    ...
                    {
                       MessageBox.Show("Invalid MO No Entered");
                    }                    
                }
                ...
                if (columnName == "Item_Code" && R1.Cells["Item_Code"].Value != null)
                {
                    ...
                    var getDetails = (from s in db.Forge_MO_Production_Status(logIn.company, R1.Cells["Job_Card"].Value.ToString(), Convert.ToInt32(R1.Cells["Item_Code"].Value.ToString()), 2)
                                      select new { s.Prod_Grade,s.QtyToCut,s.Product_Description }).FirstOrDefault();

                    R1.Cells["Heat_Code"].Value = "";
                    R1.Cells["Job_Card_Qty"].Value = "";
                    R1.Cells["Item_Name"].Value = "";

                    if (getDetails != null)
                    {
                        //  y = Convert.ToDecimal(getDetails.QtyPending.ToString());
                        R1.Cells["Heat_Code"].Value = getDetails.Prod_Grade.ToString();
                        R1.Cells["Job_Card_Qty"].Value = getDetails.QtyToCut.ToString();
                        R1.Cells["Item_Name"].Value = getDetails.Product_Description.ToString();
                        //
                    ...
                    {
                        MessageBox.Show("Invalid MO No and Item Number Entered");
                    }

                }

                if (columnName == "RF_No" && R1.Cells["RF_No"].Value != null)
                {


                    var getDetails = (from s in db.Forge_Get_bloom_cutting_data(logIn.company, R1.Cells["RF_No"].Value.ToString())
                                      select new { s.Bal_Length, s.RMSec,s.RM_WT }).FirstOrDefault();


                    R1.Cells["Cutting_Length"].Value = "0";
                    R1.Cells["RM_Sec"].Value = "0";
                    R1.Cells["Pending_Qty"].Value = "0";
                    if (getDetails != null)
                    {
                        //  y = Convert.ToDecimal(getDetails.QtyPending.ToString());
                        R1.Cells["Cutting_Length"].Value = getDetails.Bal_Length.ToString();
                        R1.Cells["RM_Sec"].Value = getDetails.RMSec.ToString();
                        R1.Cells["Pending_Qty"].Value = getDetails.RM_WT.ToString();
                        //
                    ...
                    {
                        MessageBox.Show("Invalid RF No Entered");
                    }
                ...
                if (columnName == "Qty_Produced" || columnName == "Qty_Rejected")
                {
                    ...
                    {
                        decimal Qty_Pending = ( R1.Cells["Job_Card_Qty"].Value == "" || R1.Cells["Job_Card_Qty"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(R1.Cells["Job_Card_Qty"].Value);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < dgJobCardData.Rows.Count - 1; i++)
                {
                    PlanQty =  (dgJobCardData.Rows[i].Cells["Job_Card_Qty"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["Job_Card_Qty"].Value);
                    ProdQty =  (dgJobCardData.Rows[i].Cells["Qty_Accepted"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgJobCardData.Rows[i].Cells["Qty_Accepted"].Value);
                    if (PlanQty !=ProdQty && dgJobCardData.Rows[i].Cells["Remarks"].Value == null)
                    {
                        MessageBox.Show("Production Qty is not As per Planned / Pending Qty, Enter Reason for Not Achieving The Plan");
                        return;
                    }
                ...
                
                {
                    ...
                    if ((from a in db.Forging_CuttingReport_Masters where a.Company_ID == logIn.company && a.Report_No == txtvchno.Text select a).Count() > 0)
                    {
                        ...
                        for (int i = 0; i < dgJobCardData.Rows.Count - 1; i++)
                        {
                            ...
                            SC.RF_NO = (dgJobCardData.Rows[i].Cells["RF_NO"].Value == null) ? "" : (dgJobCardData.Rows[i].Cells["RF_NO"].Value).ToString();
                            SC.C_Stage = (dgJobCardData.Rows[i].Cells["Stage"].Value == null) ? "" : (dgJobCardData.Rows[i].Cells["Stage"].Value).ToString();
            ...
            try
            {
                ...
                if (obj.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (sa.Count > 0)
                    {
                        ...
                        txtCPRef.Text = sa[0].Process;

                    }
                    var ca = (from sq in db.Forging_Cutting_Childs
                              join m in db.Forge_MFG_Order_Masters on sq.Job_CardNo equals m.MO_No
        ...
        private void txtCPRef_Leave(object sender, EventArgs e)
        {
            if (txtCPRef.Text != "")
            {

                SqlCommand cmd2 = new SqlCommand("sp_Get_CuttingPlan_Report", con);
                cmd2.CommandType = CommandType.StoredProcedure;
                cmd2.Parameters.AddWithValue("@compname", logIn.company);
                cmd2.Parameters.AddWithValue("@cpref", txtCPRef.Text);
                ...
                //           where s.PlanningRef == txtCPRef.Text && s.Company_ID == logIn.company
                ...
                //               Heat_Code = s.MaterialGrade,
                //               RM_Sec = s.RMSec,
                //               Cutting_Length= s.RM_Length,
                //               Pending_Qty = s.RM_WT,
                //               Job_Card_Qty = s.Qty,
                //               Machine_Name = c.Supplier_Name,
                //               s.Stage
