...
{
    public partial class frmDefineHeads : Form
    {
        ...
        public void bindGroups()
        {
            try
            {
                var p = (from s in db.HR_Salary_HeadsInfos where s.Status == 1 && s.Company_ID == logIn.company
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Head Name should Not be Empty", "Heads Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    txtGroupName.Focus();
                    return;
                }
                else if (txtGroupShortName.Text == string.Empty)
                {
                    MessageBox.Show("Please Enter Head Shortname", "Heads Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        public void Save()
        {
            try
            {
                    ...
                    if(txtGroupID.Text !="")
                {
                            ...
                            var c = db.HR_Salary_HeadsInfos.Where(w => w.id == Convert.ToInt32(txtGroupID.Text)).FirstOrDefault();
                        {
                            //c.ID = Convert.ToInt32(txtCityId.Text.ToString());
                            c.Head_Name = (txtGroupName.Text == "") ? "" : (txtGroupName.Text);
                            c.Short_Name = (txtGroupShortName.Text == "") ? "" : (txtGroupShortName.Text);
                            c.Calculation = cmbCalculation.Text;
                            c.basicper = Convert.ToDecimal(txtBasicPer.Text);
                            c.DisplayOrder = Convert.ToInt32(txtDispOrder.Text);
                            c.Calculate_ESI = chkESI.Checked;
                            c.Calculate_PF = chkPF.Checked;
                ...
                else
                {
                    ...
                    //{
                        HR_Salary_HeadsInfo ci = new HR_Salary_HeadsInfo();
                    ci.Head_Name = (txtGroupName.Text == "") ? "" : (txtGroupName.Text);
                    ci.Short_Name = (txtGroupShortName.Text == "") ? "" : (txtGroupShortName.Text);
                    ci.Calculation = cmbCalculation.Text;
                    ci.basicper = Convert.ToDecimal(txtBasicPer.Text);
                    ci.DisplayOrder = Convert.ToInt32(txtDispOrder.Text);
                    ci.Calculate_ESI = chkESI.Checked;
                    ci.Calculate_PF = chkPF.Checked;
                    ci.Status = 1;                    
                    ci.Company_ID = logIn.company;
                     db.HR_Salary_HeadsInfos.InsertOnSubmit(ci);
                        db.SubmitChanges();
           ...
           try
           {
                if (txtGroupID.Text != "")
                {
                        ...
                        if (result == DialogResult.Yes)
                        {
                            ...
                            SqlCommand cmd1 = new SqlCommand("delete  from [HR_Salary_HeadsInfo] where id =@ProdID and Company_ID = @compID", con);
        ...
        private void dgvcity_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                var d = (from po in db.HR_Salary_HeadsInfos
                ...
                if (d.Count > 0)
                {
                    ...
                    txtGroupName.Text = d[0].Head_Name;
                    txtBasicPer.Text = d[0].basicper.ToString();
                    txtGroupShortName.Text = d[0].Short_Name;
                    cmbCalculation.Text = d[0].Calculation;
                    txtDispOrder.Text = d[0].DisplayOrder.ToString();
                    chkESI.Checked = false;
                    chkPF.Checked = false;

                    if(d[0].Calculate_PF ==true)
                    {
                        chkPF.Checked = true;
                    }

                    if (d[0].Calculate_ESI ==true)
                    {
                        chkESI.Checked = true;
                    }
        ...
        private void txtGroupShortName_Leave(object sender, EventArgs e)
        {
