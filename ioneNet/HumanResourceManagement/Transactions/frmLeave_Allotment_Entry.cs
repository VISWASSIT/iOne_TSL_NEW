...
{
    public partial class frmLeave_Allotment_Entry : Form
    {
        DataClasses1DataContext db = new DataClasses1DataContext();
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["iOneConnection"].ConnectionString);
        public static string SONo, ItemCode, RecQty, Suppname;
        public frmLeave_Allotment_Entry()
        {
            InitializeComponent();
        }
        ...
        private void frmOpeningStock_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //Bind Heads
                var bindHeads = (from m in db.HR_Leaves_Masters
                                 where m.Company_ID == logIn.company 
                                 select new
                                 {
                                     m.Short_Name,
        ...
        private void btnGetAccounts_Click(object sender, EventArgs e)
        {
            try
            {
                if(cmbEmpGroup.Text=="")
                {
                    ...
                    SqlCommand cmd2 = new SqlCommand("SP_HR_Get_Employee_Leave_Allotment", con);
                    ...
                    cmd2.Parameters.AddWithValue("@Leavename", cmbHeadName.Text);
        ...
        private void Save()
        {
            try
            {
                if ((from u in db.HR_Leave_Allotments where u.Leave_Name == cmbHeadName.Text && u.Company_ID == logIn.company && u.BU_ID == logIn.BU_ID && u.Month == cmbMonth.Text && u.Year == cmbYear.Text && u.Emp_Group == Convert.ToInt32(cmbEmpGroup.SelectedValue) && u.Leave_Name == cmbHeadName.Text select u).Count() > 0)
                {

                    //if (AppCode.GlobalAccess.Edit == "Yes")
                    //{
                    db.sp_DeleteLeave_Allotment(cmbMonth.Text,cmbYear.Text,  logIn.company,logIn.BU_ID, Convert.ToInt32(cmbEmpGroup.SelectedValue),cmbHeadName.Text);
                ...
                //{
                    ...
                    string ed = Y + "-" + m + "-" + D;
                        ...
                        for (int i = 0; i < dgvJournalVouchar.Rows.Count - 1; i++)
                        {
                            ...
                            HR_Leave_Allotment p = new HR_Leave_Allotment();
                            ...
                            p.Leave_Name = cmbHeadName.Text;
                            p.Leave_Qty = (dgvJournalVouchar.Rows[i].Cells["No_of_Leaves"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgvJournalVouchar.Rows[i].Cells["No_of_Leaves"].Value); ;
                          
                            
                            p.Created_By = lblCreatedBy.Text;
                            p.Modified_BY = logIn.username + "-" + DateTime.Now;

                            db.HR_Leave_Allotments.InsertOnSubmit(p);
