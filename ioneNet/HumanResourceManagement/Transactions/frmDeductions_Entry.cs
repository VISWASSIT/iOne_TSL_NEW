...
namespace ioneNet.HumanResourceManagement.Transactions
{
    public partial class frmDeductions_Entry : Form
    {
        ...
        private void btnGetAccounts_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if(cmbEmpGroup.Text=="")
                {
                    ...
                    SqlCommand cmd2 = new SqlCommand("SP_HR_Get_Employee_OtherDeductions", con);
        ...
        private void Save()
        {
            try
            {
                if ((from u in db.HR_Monthly_Deductions where u.Company_ID == logIn.company && u.BU_ID == logIn.BU_ID && u.month == cmbMonth.Text && u.Year == cmbYear.Text && u.Emp_Group == Convert.ToInt32(cmbEmpGroup.SelectedValue) select u).Count() > 0)
                {

                    //if (AppCode.GlobalAccess.Edit == "Yes")
                    //{
                    db.sp_DeleteOther_Deductions (cmbMonth.Text,cmbYear.Text,  logIn.company,logIn.BU_ID, Convert.ToInt32(cmbEmpGroup.SelectedValue));
                ...
                //{
                    ...
                    string ed = Y + "-" + m + "-" + D;
                        ...
                        for (int i = 0; i < dgvJournalVouchar.Rows.Count - 1; i++)
                        {
                            ...
                            HR_Monthly_Deduction p = new HR_Monthly_Deduction();
                            ...
                            p.ESI = (dgvJournalVouchar.Rows[i].Cells["ESI"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgvJournalVouchar.Rows[i].Cells["ESI"].Value); ;
                          
                            p.PF = (dgvJournalVouchar.Rows[i].Cells["PF"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgvJournalVouchar.Rows[i].Cells["PF"].Value);
                            p.ProfTax = (dgvJournalVouchar.Rows[i].Cells["ProfTax"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgvJournalVouchar.Rows[i].Cells["ProfTax"].Value);
                            p.TDS_Amount = (dgvJournalVouchar.Rows[i].Cells["TDS_Amount"].Value == DBNull.Value) ? Convert.ToDecimal("00") : Convert.ToDecimal(dgvJournalVouchar.Rows[i].Cells["TDS_Amount"].Value);
                            p.BU_ID = logIn.BU_ID;
                            p.Created_By = lblCreatedBy.Text;
                            p.Modified_BY = logIn.username + "-" + DateTime.Now;

                            db.HR_Monthly_Deductions.InsertOnSubmit(p);
